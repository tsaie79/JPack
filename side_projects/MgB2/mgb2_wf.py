...
def c_terminate_mgb2(cat="slab_dipol"):
    ...
    for st in glob.glob("/home/tug03990/config/project/mgb2/slab/structure/mv/*.vasp".format(cat)):
        ...
        opt_0 = JSelectiveOptFW(init_st, selective_dynamics=two_layers_from_top,
        ...
        opt_1 = JSelectiveOptFW(init_st, selective_dynamics=two_layers_from_top, parents=opt_0,
                                force_gamma=True, vasptodb_kwargs={"parse_eigenvalues":False}, name="{}_opt_1".format(st_name))


        scf_fw = StaticFW(init_st, parents=opt_1, vasptodb_kwargs={"parse_eigenvalues":False}, name="{}_scf".format(st_name))

        fws = [opt_0, opt_1, scf_fw]
        wf = Workflow(fws, name="{}:{}:sel".format(st_name, cat))
        wf = add_modify_incar(
            wf,
            {
                "incar_update": {
        ...
        wf = add_additional_fields_to_taskdocs(wf, {"cat": cat, "stack": st_name, "selective":two_layers_from_top})
