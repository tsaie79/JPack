...
plt = dsplot.get_plot([-6,6])
plt.show()
#%%
proj_path = '/Users/jeng-yuantsai/Research/project/symBaseBinaryQubit/calculations/search_triplet_from_defect_db'
db_host_json = os.path.join(proj_path, "db.json")
db = VaspCalcDb.from_db_file(db_host_json)
ev = db.get_eigenvals(1920)
for i in [448, 447, 446, 445]:
    print(i, ev["1"][0][i], ev["-1"][0][i])
...
proj_path = '/Users/jeng-yuantsai/Research/project/symBaseBinaryQubit/calculations/search_triplet_from_defect_db'
db = VaspCalcDb.from_db_file(os.path.join(proj_path, "db.json"))
col = db.collection.find({"task_label":"SCAN_scf"})
dirs = [ e["calcs_reversed"][0]["dir_name"].split("/")[-1] for e in col]

from monty.serialization import dumpfn

dumpfn(dirs, "dirs.json")
